...
class PipelineState(TypedDict, total=False):
    ...
    # --- ranking output (per-track) ---
    query_ranked: List[dict]
    seed_ranked: List[dict]

    # --- ranking-critic output ---
    # ranking_critic_node picks ONE track to carry forward (seed wins in
    # hybrid mode -- no merge for now), then runs an LLM-as-judge over its
    ...
    # here works on the one `ranked` list.
    ranked: List[dict]
